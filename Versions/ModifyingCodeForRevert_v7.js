class Node {
  ...
  let otherOperationsDisableFlag = false;
  ...
  function userDefinedList(){
    ...
    submitButton.mouseClicked(function() {
      ...
      if (inputBox.value() !== '') {
        ...
        size=list.length;
        otherOperationsDisableFlag=false;
        pointer=0;
  ...
  function forwardAnimation(){
    ...
    if((insertHead || insertTail || removeHead || insertRemoveAnyStart) && otherPointer<3 && !removeTailStart && !removeAny){
      otherPointer+=1;
      if(otherPointer==1 && (insertHead || insertTail || insertRemoveAnyStart)){
        forwardButton.attribute('disabled',true);
        revertButton.attribute('disabled',true);
        pauseButton.attribute('disabled',true);
      }
  ...
  function deploy(pointer){
    if(list[pointer][5]==0 && calculated && isPlaying){
        if(pointer==0){
          forwardButton.attribute('disabled',true);
          revertButton.attribute('disabled',true);
          pauseButton.attribute('disabled',true);
        }
        if(!otherOperationsDisableFlag){
          createListButton.attribute('disabled', true);
          searchButton.attribute('disabled', true);
          insertButton.attribute('disabled', true);
          removeButton.attribute('disabled', true);
          otherOperationsDisableFlag=true;
        }
        console.log(elapsedTime);
        let progress = constrain((elapsedTime - animationGap) / animationDuration,0,1);
        let animatedDiameter = lerp(0,diameter,progress);
        list[pointer][1] = 0;
        let colorCode = decodeColor(0);
        fill(colorCode);
        ellipse(list[pointer][3], list[pointer][4], animatedDiameter, animatedDiameter);
        fill(0,0,0);
        noStroke();
        textAlign(CENTER, CENTER);
        ...
        if(elapsedTime>animationDuration+100){
          forwardButton.removeAttribute('disabled');
          revertButton.removeAttribute('disabled');
        ...
        else if(!playPause && elapsedTime>animationSpacing){
            isPlaying=false;
        }
    }
    ...
    if(list[pointer][5]==1 && searchingList && isPlaying){
      ...
      if(list[pointer][2]==searchNo && elapsedTime>animationSpacing+500){
        updateAlgorithm(4);
        if(otherOperationsDisableFlag){
          createListButton.removeAttribute('disabled');
          searchButton.removeAttribute('disabled');
      ...
      else if(list[pointer][2]!==searchNo && elapsedTime>animationSpacing+500){
        updateAlgorithm(1);
        forwardButton.removeAttribute('disabled');
        revertButton.removeAttribute('disabled');
        pauseButton.removeAttribute('disabled');
        ...
        if(playPause && elapsedTime>animationSpacing+500+animationDuration){
          console.log("Running this...");
            isPlaying=false;
            forwardAnimation();
        }
        else if(!playPause && elapsedTime>animationSpacing+500+animationDuration){
            isPlaying=false;
        }
      }
    ...
    if(insertHead && isPlaying){
      ...
      if(otherPointer==1){
        ...
        fill(colorCode);
        ellipse(tempArray[3], tempArray[4], animatedDiameter, animatedDiameter);
        fill(0,0,0);
        noStroke();
        textAlign(CENTER, CENTER);
        let textS = lerp(0,20,progress);
        ...
        else if(!playPause && elapsedTime>animationSpacing + animationGap){
          isPlaying=false;
        }
      }
    ...
    if(insertTail && isPlaying){
      ...
      if(otherPointer==1){
        ...
        if(elapsedTime>animationDuration+300){
          forwardButton.removeAttribute('disabled');
          revertButton.removeAttribute('disabled');
          pauseButton.removeAttribute('disabled');
        }
    ...
    if(insertAny && isPlaying){
      if(pointer===0 && !insertRemoveAnyStart){
        ...
        updateAlgorithm(2);
        fill(200);
        ellipse(temporaryArray[3], temporaryArray[4] + diameter + spacing, animatedDiameter, animatedDiameter);
        fill(0,0,0);
        noStroke();
        textAlign(CENTER, CENTER);
        let textS = lerp(0,20,progress);
  ...
  function diminisher(pointer){
    ...
    if(insertTail){
      ...
      if(otherPointer==0){
        ...
        fill(0);
        if(insertedRemovedFlag){
          rect(list[list.length - 1][3] - (diameter / 2) - 2, list[list.length - 1][4] - (diameter / 2) - 2, diameter + 4, diameter + spacing + 7);
        }
        else{
          rect((startX + size*(diameter + spacing)) - (diameter / 2) - 2, yAxis - (diameter / 2) - 2, diameter + 4, diameter + spacing + 7);
        }
        stroke(200);
      }
    }
